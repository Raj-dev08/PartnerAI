...
export const subscribePlan = async (req, res, next) => {
    try {
        ...
        const result = await sql.begin( async (tx) => {
            ...
            return { subscription: subscription[0], payment: payment[0] };

        })

        await User.findOneAndUpdate(
                { _id: user._id },
                { $set: { subscription: result.subscription.id } },
                { new: true }
        );
    ...
    try {
        ...
        await sql.begin( async (tx) => {
            ...
            const subscription = await tx`
                ...
                WHERE id = ${payment[0].subscription_id} AND u_id = ${user._id}
                FOR UPDATE
            `;
...
export const getUserSubscriptionAndPayment = async (req, res, next) => {
    try {
        ...
        `;
        //Getting all the ones from past to add inactive bar maybe 
        if (!subscription || subscription.length === 0) {
    ...
    try {
        ...
        await sql.begin( async (tx) => {
            const subscription = await tx`
                ...
                JOIN plans p ON p.id = s.plan_id
                WHERE id = ${user.subscription} AND u_id = ${user._id}
                FOR UPDATE
            `;
